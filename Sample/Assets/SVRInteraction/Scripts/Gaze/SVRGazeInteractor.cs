...
namespace Skyworth.XR.Interaction.Gaze
{
...
public class SVRGazeInteractor : MonoBehaviour
{
    ...
    private bool AndroidGlobalMenuOpened = false;
    ...
    void Update()
    {
        ...
        if (!SVROrigin.Instance.LHandConnect && !SVROrigin.Instance.RHandConnect && SVROrigin.Instance.ControllerVisible)
        {
            m_XRRayInteractor.enabled = true;
            rendererComponent.enabled = true;
#if SVR_V920
            //if (_svrOpenxrFeature == null)
            //    _svrOpenxrFeature = OpenXRSettings.Instance.GetFeature<SvrOpenxrFeature>();
            //if (_svrOpenxrFeature)
            //    AndroidGlobalMenuOpened = _svrOpenxrFeature.GetGlobalMenuOpened();
            if (SystemProperties.get("sxr.globalmenu.open", "0") == "1")
                AndroidGlobalMenuOpened = true;
            else
                AndroidGlobalMenuOpened = false;
#endif
            if (AndroidGlobalMenuOpened)
            {
