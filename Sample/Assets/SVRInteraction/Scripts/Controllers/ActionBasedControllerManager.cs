...
public class ActionBasedControllerManager : MonoBehaviour
{
    ...
    public bool TransitionOrTurnEnable = true;
...
#pragma warning restore IDE0044
    ...
    SVRRayInteractor _mSvrRayInteractor;
    ...
    void FindBaseControllerComponents()
    {
        ...
        if (_mSvrRayInteractor == null)
        {
            _mSvrRayInteractor = m_BaseControllerGameObject.GetComponent<SVRRayInteractor>();
            if (_mSvrRayInteractor == null)
                Debug.LogWarning($"Cannot find any {nameof(SVRRayInteractor)} component on the Base Controller GameObject.", this);
        }
    ...
    {
        Debug.Log($"Setting base {gameObject.name} controller to {enable}");
        FindBaseControllerComponents();

        if (m_BaseController != null)
        {
            m_BaseController.enableInputActions = enable;
            m_BaseController.modelParent.gameObject.SetActive(enable);
        }
        ...
        if (m_BaseInteractor is XRRayInteractor && m_BaseLineVisual != null){
            ...
            Debug.Log($"Setting base {gameObject.name} line visual to {enable}");
        }
    }
    ...
    void SetTeleportController(bool enable)
    {
        ...
        if (m_TeleportController != null)
        {
            m_TeleportController.modelParent.gameObject.SetActive(enable);
    ...
    void OnUpdateSelectState()
    {
        ...
        if (_mSvrRayInteractor != null)
        {
            overUI = _mSvrRayInteractor.IsOverUIGameObject();
        }

        if (overUI || !TransitionOrTurnEnable)
        {
            DisableAction(m_Turn);
        }
        else
        ...
        if (triggerTeleportMode && !cancelTeleport && !overUI && TransitionOrTurnEnable)
        {
    ...
    void OnUpdateTeleportState()
    {
        ...
        if (cancelTeleport || releasedTeleport || !TransitionOrTurnEnable)
        {
