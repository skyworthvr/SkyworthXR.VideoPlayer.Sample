...
public class SvrVideoPlayerDemo : MonoBehaviour
{
    ...
    public void PlayVideoByIndex(int index)
    {
        ...
        if(isLive)
            SvrVideoPlayer.PreparedPlayLiveVideo(VideoUrls[currentPlayingVideoIndex]);
        else
    ...
    private void OnVideoError(ExceptionEvent errorCode, string errMessage,int percent)
    {
        Debug.LogErrorFormat("{0}:{1}:{2}", errorCode.ToString(), errMessage, percent);
    }
    ...
    public void SwitchResolution()
    {
        ...
        if (currentPlayingVideoIndex + 1 > videoCount - 1)
            currentPlayingVideoIndex = 0;
        else if (currentPlayingVideoIndex + 1 < 0)
            currentPlayingVideoIndex = videoCount - 1;
        else
            currentPlayingVideoIndex += 1;

        SvrVideoPlayer.SwitchResolution(VideoUrls[currentPlayingVideoIndex],0);
    }
}
